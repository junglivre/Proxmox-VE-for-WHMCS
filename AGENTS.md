...
- A versão é duplicada em `modules/addons/pvewhmcs/pvewhmcs.php` e no arquivo raiz `version`; mantenha ambos sincronizados. **Não incremente a versão a cada commit**: acumule fixes e features sob a versão atual (adicionando ao mesmo bloco de `pvewhmcs_upgrade()` e à mesma entrada de `CHANGELOG.md`) até o usuário pedir explicitamente para liberar/subir a versão. **Ressalva:** o WHMCS só chama `pvewhmcs_upgrade()` quando a versão registrada em `tbladdonmodules` muda; se você adicionar uma migração nova dentro do MESMO bloco de versão depois que essa versão já foi ativada num WHMCS, ela nunca roda sozinha ali. Nesse caso, dê ao usuário o `ALTER TABLE`/`CREATE TABLE IF NOT EXISTS` equivalente pra rodar manualmente (mesmo texto do bloco de migração), já que reativar o módulo também não adiciona coluna em tabela existente.
